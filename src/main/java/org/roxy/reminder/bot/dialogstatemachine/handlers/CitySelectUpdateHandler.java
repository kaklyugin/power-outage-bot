...
public class CitySelectUpdateHandler implements UpdateHandler {
    ...
    public boolean isApplicable(UpdateDto update, ChatContext context) {
        return context.getStatus().equals(DialogStatus.WAITING_FOR_CITY_INPUT)
                && update.getSourceMessageId().equals(context.getLastBotMessageId());
    ...
    public void handleUpdate(UpdateDto update, ChatContext context) {
        ...
        context.setCity(update.getUserResponse());        // Чтобы убрать blur кнопки
        ...
        MessageDto cityInputQuestionMessage = MessageDto.builder()
                ...
                .text("Введите улицу")
                .build();
        Long messageId = botClient.sendMessage(cityInputQuestionMessage).getMessageId();
        context.setStatus(DialogStatus.WAITING_FOR_STREET_INPUT);
        log.info("Sent street message id = {}", messageId);
    }
}
